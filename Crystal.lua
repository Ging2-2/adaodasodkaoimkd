...
yes i know the formatting is shitty, cause of some lua beautifier
its also cuz i copied it off some random pastebin that took it from my original v3rm thread which is now gone cuz v3rm gone
        ...
        Callback = function(v)
            if v then
                game.StarterGui:SetCore(
                    "SendNotification",
                    {
                        Title = "AutoGaurd/Defend",
                        Text = "Press U to go to the nearest ball",
                        Duration = 5,
                        Button1 = "Okay"
                    }
                )
...
function stepped()
    pcall(
        function()
            ...
            if
                _G.Autogaurd and tracking and player and plr.Character and plr.Character:FindFirstChild("Humanoid") and
                    not plr.Character:FindFirstChild("Basketball") and
